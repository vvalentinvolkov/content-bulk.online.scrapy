...
def test_bulk_fields_query(client, set_test_document):
    ...
    resources.MODEL_CLASSES['a'] = set_test_document(10)
    response = client.get('/a?fields=fa_int;fa_str').get_json()
...
def test_bulk_limit_page_query(client, set_test_document):
    ...
    limit = 2
    page = 2
    response = client.get(f'/a?limit={limit}&page={page}').get_json()
...
def test_bulk_sort_query(client, set_test_document):
    ...
    resources.MODEL_CLASSES['a'] = set_test_document(10)
    sort = 'fa_int__d'
    response = client.get(f'/a?limit=10&sort={sort}').get_json()
    for i, item in enumerate(response):
        assert item['fa_int'] == len(response) - 1 - i

    sort = 'fa_int'
    response = client.get(f'/a?sort={sort}').get_json()
    for i, item in enumerate(response):
...
def test_bulk_filter_query(client, set_test_document):
    ...
    response = client.get(f'/a?limit=10&filters={filter_1};{filter_2};{filter_3}').get_json()
